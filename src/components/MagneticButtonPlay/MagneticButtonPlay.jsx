import "./MagneticButtonPlay.scss";
import gsap, { Power4, Elastic } from "gsap";
import { useEffect } from "react";
import { triggerHideAnimation } from "../../utils/gsap/animations";

const MagneticButtonPlay = (props) => {
  const { title, click = () => {}, fontSize } = props;
  useEffect(() => {
    const magnet = document.querySelector(".magnetic-created-play");
    const text = document.querySelector(".btn-text-created-play");
  ...
  return (
    <div
      className="wrapper-created-play"
      onClick={() => {
        click();
    ...
    >
      <div
        className="btn-click-created magnetic-created-play"
        data-strength="25"
        data-strength-text="2"
      >
        <span className="btn-text-created-play" style={{ fontSize: fontSize }}>
...
export default MagneticButtonPlay;
